...
import { getChatSearchKeyword } from './chat-search.js?v=temp';
...
import { openAvatarPreview, DEFAULT_AVATAR } from './modals.js?v=temp';
import { extractThinkingFromContent, parseStreamingThinkContent, jsonToMarkdownTable, isFloorHiddenInConfig, isMessageHidden } from './utils.js?v=temp';
import CodeBlockEnhancer from './code-block-enhancer.js?v=temp';
import { regexPatterns } from './regex.js?v=temp';

// --- Sub-system Imports ---
import { initSharedStyleSheets, getSharedStyleSheets, allShadowRoots } from './renderers/style-manager.js?v=temp';
import { updateDomPreservingCodeBlocks } from './renderers/stream-renderer.js?v=temp';
import { updateReasoningContainer, updateReasoningPartContent } from './renderers/reasoning-renderer.js?v=temp';
import { formatMessagePipeline, enhanceJsonCodeBlocks } from './renderers/markdown-engine.js?v=temp';
// Re-export UI populators for compatibility with main.js/settings-events.js
export { populateApiSelector, populatePersonaSelector, renderApiEndpointsList, renderPersonaModal, renderRegexRulesList } from './ui-populator.js?v=temp';
...
export async function displayMessage(message, index, highlightKeyword, visibleIndex, totalVisibleMessages, options = {}) {
        ...
        if (message.toolCalls && Array.isArray(message.toolCalls) && message.toolCalls.length > 0) {
            ...
            badge.addEventListener('click', () => {
                ...
                if (contentEl && contentEl.classList.contains('collapsible')) {
                    import('./message-manager.js?v=temp').then(module => {
    ...
    if (message.toolCalls && Array.isArray(message.toolCalls)) {
        ...
        message.toolCalls.forEach((toolCallResult, index) => {
            if (toolCallResult && toolCallResult.success !== undefined) {
                import('./mcp-renderer.js?v=temp').then(module => {
    ...
    if (message.analysisRounds && message.analysisRounds.length > 0) {
        import('./mcp-renderer.js?v=temp').then(async ({ renderToolCallResult }) => {
        ...
        // 乌鸦：旧数据兼容——单个分析容器
        import('./mcp-renderer.js?v=temp').then(async () => {
