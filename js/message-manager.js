...
import {extractJsonArrayString, copyTextToClipboard, countTokens, escapeHtml} from './utils.js?v=temp';
import {openMessageEditModal} from './modals.js?v=temp';
...
export async function switchMessageVersion(message, targetIdx, bubbleIndex) {
    ...
    const { saveConversation } = await import('./db.js?v=temp');
    ...
    if (messageElement) {
        ...
        if (contentEl) {
            const { formatMessagePipeline, renderFormattedContent } = await import('./renderer.js?v=temp');
