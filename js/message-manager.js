...
import {extractJsonArrayString, copyTextToClipboard, countTokens, escapeHtml} from './utils.js';
import {openMessageEditModal} from './modals.js';
...
export async function switchMessageVersion(message, targetIdx, bubbleIndex) {
    ...
    if (messageElement) {
        ...
        if (contentEl) {
            const { formatMessagePipeline, renderFormattedContent } = await import('./renderer.js');
