...
import { getAvatarUrl } from './renderer.js?v=temp';
import { DEFAULT_AVATAR } from './modals.js?v=temp';
...
function createHistoryItem(convId, conv) {
    ...
    const actionsHtml = state.batchSelectMode ? '' : `
        <div class="actions">
            ...
            <button class="more-conv-btn" title="更多操作" aria-label="更多操作"><svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="1.5"></circle><circle cx="19" cy="12" r="1.5"></circle><circle cx="5" cy="12" r="1.5"></circle></svg></button>
        </div>
    `;
...
async function getFirstTwoMessagesPreview(conv, maxLength = 200) {
    ...
    if (!targetConv.branches && targetConv.id) {
        try {
            const { getConversation } = await import('./db.js?v=temp');
