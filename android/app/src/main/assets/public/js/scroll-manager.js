...
class ScrollManager {
    ...
    setupScrollListeners() {
        ...
        // wheel 滚轮事件：立即锁定，wheelLocked + immediateWheelLock 双保险
        // immediateWheelLock 在同一事件循环中生效，用于阻断已经在队列中的 smartScrollToBottom
        dom.chatMessages.addEventListener('wheel', () => {
            // 同步设置，同一事件循环内生效
            ...
            }, 300); // 乌鸦：延长到300ms，以完全覆盖系统的鼠标滚轮平滑滚动动画时长
        }, { passive: true });
    }
    ...
    smartScrollToBottom(forceIfWasAtBottom = false) {
        ...
        // — 为什么这么写 —
        // 1. immediateWheelLock 是同步急停，同一事件循环内必须生效，forceIfWasAtBottom 也不能覆盖
        // 2. isUserScrolling 是物理操作（手指/鼠标按着拖），forceIfWasAtBottom 也不能覆盖
        // 3. wheelLocked 是滚轮锁定，但 forceIfWasAtBottom 可以覆盖它！
        //    因为：用户快速滚到底部时，每一帧 wheel 事件都重设 wheelLocked=true，
        //    如果 forceIfWasAtBottom 不能覆盖，那渲染前在底部 + 渲染后内容增长 = 永远不滚 = 闪烁
        //    forceIfWasAtBottom 的语义是"渲染前就在底部，渲染后理应继续跟随"，
        //    这比 wheelLocked 的"用户滚过轮"优先级更高
        if (this.immediateWheelLock) return false;
        if (this.isUserScrolling) return false;
        if (this.wheelLocked && !forceIfWasAtBottom) return false;
