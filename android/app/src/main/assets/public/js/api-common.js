...
export function filterMessagesByHideSummary(allMessages, hideSummaryConfig, activeBranch) {
    ...
    const hideSummaryConfigLocal = state.hideSummary && state.hideSummary[convId];
    let hideStart = 0, hideEnd = -1, hideEnabled = false;
    if (hideSummaryConfigLocal && hideSummaryConfigLocal.enabled) {
        hideStart = Number(hideSummaryConfigLocal.start) || 1;
        hideEnd = Number(hideSummaryConfigLocal.end) || hideStart;
        hideEnabled = true;
    }

    if (hideEnabled && hideSummaryConfigLocal && hideSummaryConfigLocal.summary && hideSummaryConfigLocal.summary.trim()) {
        // 隐藏模式（含总结）：用总结替换隐藏楼层区间
        const messagesBefore = allMessages.filter(m => m._idx < hideStart);
        const summaryMessage = { role: 'system', content: '本条消息属于对历史对话内容的总结，你必须阅读后作为记忆参考再进行回答：'+hideSummaryConfigLocal.summary.trim()+'' };
        const messagesAfter = allMessages.filter(m => m._idx > hideEnd);
        return [...messagesBefore, summaryMessage, ...messagesAfter];
    } else {
        // 正常模式或隐藏但无总结：过滤掉所有被标记隐藏的楼层（支持离散与区间）
        return allMessages.filter(msg => {
            if (!hideEnabled) return true;
            return !isFloorHiddenInConfig(msg._idx, hideSummaryConfigLocal);
        });
    }
}
